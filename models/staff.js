...
export async function fetchFilteredAchievements(filters, page = 1, pageSize = 10) {
  try {
    ...
    if (filters.from) query.date = { ...query.date, $gte: new Date(filters.from) };
    if (filters.to) query.date = { ...query.date, $lte: new Date(filters.to) };
    ...
    const selectFields = filters.select && Array.isArray(filters.select)
      ? filters.select.join(' ')
      : '_id achievementName date rollNumber category';
